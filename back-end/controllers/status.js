...
module.exports = function() {
    ...
    controller.listar = function(req, res) {
        Status.find().exec().then( //populate('tipo') -> para popular com as informações da tabela tipo
    ...
    controller.obterUm = function(req, res) {
        ...
        Status.findById(req.params.id).exec().then(
            function(status) {
                if(status) { //Se o artigo existe (não veio vazio)
    ...
    controller.atualizar = function(req, res) {
        ...
        Status.findByIdAndUpdate(req.body._id, req.body).exec().then(
            function() {
    ...
    controller.excluir = function(req,res) {
        ...
        Status.findByIdAndRemove(req.params.id).exec().then(
            function() {
                res.send(204).end();
            },
            function(erro) {
